...
export const Signup1 = async (req, res) => {
  try {
    ...
    if (!user) {
      console.log(
        "Signup1 - User not found for mobile:",
        mobile,
        "Creating new user."
      );
    ...
    // Update session progress if session token is provided
    const sessionToken = req.headers["x-session-token"];
    if (sessionToken) {
      try {
        const sessionService = new SessionService(req.conn1);
        await sessionService.updateSessionSignupProgress(
          sessionToken,
          1,
          false
        );
...
export const Signup3 = async (req, res) => {
  try {
    ...
    if (sessionToken) {
      try {
        const sessionService = new SessionService(req.conn1);
        await sessionService.updateSessionSignupProgress(sessionToken, 3, true);
      } catch (sessionError) {
        console.error("Error updating session progress:", sessionError);
        // Don't fail the request if session update fails
      }
