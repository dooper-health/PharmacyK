...
const Earning = () => {
  ...
  useEffect(() => {
    const fetchEarnings = async () => {
      try {
        const response = await axios.post('/api/earning2/total2', { phoneNumber: mobileNumber });
  ...
  useEffect(() => {
    const fetchEarnings2 = async () => {
      try {
        const response = await axios.post('/api/earning2/available2', { phoneNumber: mobileNumber });
      ...
      <main className=' sm:mx-[120px] pt-[24px] pb-[48px] overflow-x-hidden'>
        ...
        <Link to="/myprofile">
          ...
          <div className='flex flex-wrap'>
            <div className='sm:p-[20px] p-[10px] sm:bg-white bg-gray-100  sm:w-[200px] w-[150px] sm:mt-[16px] mt-[30px] rounded-lg sm:ml-0 ml-4 '>
            ...
            <div className='sm:p-[20px] p-[10px] sm:bg-white bg-gray-100  sm:w-[200px] w-[150px] sm:mt-[16px] mt-[30px] rounded-lg ml-4 '>
        ...
        <div className='flex'>
        <li
          className={`px-4 py-[10px] cursor-pointer ${selectedItem === 'Urgent' ? ' border-b  text-black border-[#E40443]' : ''}`}
        ...
        <div>
        <li>
        <div className='flex font-Montserrat font-medium sm:text-[18px] text-[14px] text-[#E40443] sm:mt-10 mt-3'>Sort by <img src={ud} alt="s" className='ml-1' /></div>
        </li>
        </div>
        
      </ul>
      <div className='sm:-mt-[23px] mt-5'>
