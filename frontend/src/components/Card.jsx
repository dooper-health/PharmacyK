...
const Card = () => {
  ...
  const fetchBookings = () => {
    axios.post('/api/booking2/pending2', { phoneNumber: mobileNumber })
      .then(response => {
  ...
  return (
    <>
      {bookingsUrgent.map((bookingUrgent) => (
        <Link to={`/addreportcard2/${bookingUrgent._id}`} key={bookingUrgent._id}>
        <div key={bookingUrgent._id} className='w-[320px] sm:h-[190px] h-[170px] border-[1px] border-[#D3D3D3] rounded-xl mt-[24px] mx-auto'>
          <div className="flex flex-col border-b-[1px] border-[#E4E4E4] px-[16px] py-[6px] top-0 justify-center items-start ">
            <div className='w-[300px] flex justify-between'>
              <h1 className='text-[#E40443] font-Montserrat text-[14px] font-[600] break-words'>{bookingUrgent.bookingId}</h1>
            ...
            <img src={pro} alt="pro" className='' />
             ...
             <div className='flex justify-between'>
              ...
              <span className='font-Montserrat mb-[2px] h-[20px] w-[48px] rounded-3xl text-[8px] font-medium bg-red-100 py-1 text-center'>{bookingUrgent.status}</span>
              </div>
              <div className='flex justify-between'>
