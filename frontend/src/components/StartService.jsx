...
//                 <h2 className='font-Montserrat text-[12px] text-[#5B6572] font-normal mt-[19px]'>
//                   {/* {bookingDetails.startServiceDate} */}
...
//                 <h2 className='font-Montserrat text-[12px] text-[#5B6572] font-normal mt-[19px]'>
//                   {/* {bookingDetails.sampleCollectedDate} */}
...
//     try {
//       axios.post(`/api/Dosage/${bookingId}`, { dosage })
//         .then(response => {
...
//     try {
//       const response = axios.post(`/api/otp/sendotp/${bookingId}`)
//       .then(response => {
...
const EndService = () => {
  ...
  const handleEndService = () => {
    axios.post(`/api/sd/completedmedicine/${bookingId}`)
      .then(response => {
        alert('Service completed successfully');
      })
      .catch(error => {
  ...
  const handleDosage = async () => {
    try {
      axios.post(`/api/dosage/1/${bookingId}`, { dosage })
        .then(response => {
          alert('Dosage created successfully');
        });
    } catch (error) {
  ...
  return (
    <>
      <div className='w-full '>
        ...
        {showOtpCard && <OtpCard bookingId={bookingId} />}
        <Footer />
      </div>
    </>
  );
