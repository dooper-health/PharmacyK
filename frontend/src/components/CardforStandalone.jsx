...
const CardforStandalone = () => {
  ...
  const fetchBookings = () => {
    axios.post('/api/booking2/incoming2', { phoneNumber: mobileNumber })
      .then(response => {
  ...
  const handleAccept = (id) => {
    axios.post(`/api/booking2/${id}/accept2`)
      .then(response => {
  ...
  const handleReject = (id) => {
    axios.post(`/api/booking2/${id}/reject2`)
      .then(response => {
        setBookingsUrgent(bookingsUrgent.filter(booking => booking._id !== id));
      })
      .catch(error => {
...
export default CardforStandalone;
