...
const EnhancedFirebaseOtpCard = ({ 
  bookingId, 
  phoneNumber, 
  onSuccess, 
  onError, 
  serviceType = 'medicine' // 'medicine' or 'vaccine'
}) => {
  ...
  const [verificationStatus, setVerificationStatus] = useState('pending'); // pending, sent, verified, failed
  ...
  const checkVerificationStatus = async () => {
    try {
      const response = await axios.get(`/api/firebase-otp/status/${bookingId}`);
  ...
  const initiateOtpVerification = async () => {
    ...
    try {
      // Initialize OTP verification on backend
      await axios.post('/api/firebase-otp/initiate', {
        bookingId,
        phoneNumber
      });
  ...
  const handleEndService = async () => {
    try {
      const endpoint = serviceType === 'vaccine' 
        ? `/api/sd/completevaccine/${bookingId}`
        : `/api/sd/completemedicine/${bookingId}`;
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const firebaseResult = await firebaseOtpService.verifyOtp(otpString);
      ...
      if (firebaseResult.success) {
        ...
        // Verify with backend
        const backendResult = await axios.post('/api/firebase-otp/verify', {
          bookingId,
          idToken
        });
        ...
        if (backendResult.data.success) {
          ...
          setVerificationStatus('verified');
          setShowSuccessCard(true);
          if (onSuccess) onSuccess(firebaseResult.user);
        } else {
          setErrorMessage(backendResult.data.message);
          setVerificationStatus('failed');
        }
      } else {
    ...
    <div className='absolute top-0 left-0 w-full h-[1215px] flex items-center justify-center bg-black bg-opacity-70 z-30'>
      <div className='absolute -ml-[208px] left-1/2 transform -translate-x-1/2 -translate-y-1/2 mt-[96px] z-30'>
        <div className='sm:w-[600px] w-[300px] mb-40 h-auto p-[32px] bg-white rounded-2xl ml-[420px] relative z-30'>
          ...
          {/* Status Indicator */}
          <div className={`text-center text-sm font-medium mb-4 ${getStatusColor()}`}>
            {getStatusText()}
          </div>
            ...
            {otp.map((value, index) => (
              <div key={index} className="relative">
                <input
                  ...
                  className='border font-Montserrat relative border-gray-300 rounded-md w-[48px] h-[48px] text-center text-[16px] font-[500] focus:border-[#E40443] focus:outline-none disabled:opacity-50'
                  maxLength="1"
                  ref={inputRefs[index]}
            ...
            {!otpSent ? (
              ...
              <>
                <button
                  ...
                  disabled={isLoading || otp.join('').length !== 6 || verificationStatus === 'verified'}
                  className='inline-block bg-[#E40443] w-full pt-[12px] items-center h-[44px] rounded-lg font-Montserrat text-[14px] font-[600] text-white disabled:opacity-50 disabled:cursor-not-allowed'
                >
                ...
                <button
                  onClick={resendOtp}
                  disabled={isLoading || countdown > 0 || verificationStatus === 'verified'}
...
export default EnhancedFirebaseOtpCard; 