        ...
        <div className='float-right -mt-5 font-Montserrat text-[14px] font-semibold flex'><img src={arr} alt="" className='ml-2 mt-[2px] text-[#8D98A4] h-4 w-4'/></div>
          ...
          {isAvailable ? (
            <div className='grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 mb-5 gap-5'>
              {selectedItem === 'Standalone' && <Card />}
              {selectedItem === 'Urgent' && <BookingCard/>}
...
<div className="w-full bg-opacity-40 overflow-x-hidden scrollbar-hide">
  <Navbar />
  <main className="h-auto mx-[5px] sm:mx-[120px] pt-[24px] pb-[140px] sm:pb-0">
    <div className="flex flex-col sm:flex-row justify-between">
      <div className="flex space-x-4 sm:space-x-0">
        <Link to="/Sidebarmobile">
          <img src={hamburger} className="w-[20px] h-[26px] sm:hidden" />
        </Link>
        <div className="font-Montserrat font-[700] text-[20px] sm:text-[22px] h-[32px] pb-[8px]">
          My Booking
        </div>
      </div>
    ...
    <ul className="flex items-start flex-1 space-x-[1px] space-y-4 text-[16px] font-Montserrat text-[#8D98A4] font-[500]">
      <div
        className={`px-4 mt-4 cursor-pointer ${selectedItem === 'Urgent' ? 'border-b pb-[8px] text-[#E40443] border-[#E40443]' : ''}`}
        onClick={() => handleItemClick('Urgent')}
      >
    ...
    <div className="flex justify-end -mt-5 font-Montserrat text-[14px] font-semibold">
      <img src={arr} alt="" className="ml-2 mt-[2px] text-[#8D98A4] h-4 w-4" />
    </div>
    <hr className="mt-[10px] border border-[#E1E5E8]" />
    <Link to="">
      {isAvailable ? (
        <div className="grid gap-5 mb-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 justify-center justify-between justify-items-center ">
        {selectedItem === 'Standalone' && <Card className="w-full" />}
        {selectedItem === 'Urgent' && <BookingCard className="w-full" />}
      </div>
      ) : (
        <div className="text-center mt-4 h-[100px]">
          Availability is disabled. Please enable your availability to see the available details.
        </div>
      )}
    </Link>
  </main>
