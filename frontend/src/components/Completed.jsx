...
import CompletedMedicineCard from './CompletedMedicineCard.jsx'
import CompletedVaccineCard from './CompletedVaccineCard.jsx'
    ...
    return (
      ...
      <div className='w-full  h-[1024px] scrollbar-hide overflow-x-hidden '>
        ...
        <div className='float-right -mt-5 font-Montserrat text-[14px] font-semibold flex'><img src={arr} alt="" className='ml-2 mt-[2px] text-[#8D98A4] h-4 w-4'/></div>
          ...
          {isAvailable ? (
            <div className=''>
              {selectedItem === 'Urgent' && <CompletedMedicineCard/>}
              {selectedItem === 'Standalone' && <CompletedVaccineCard/>}
            </div>
          ) : (
